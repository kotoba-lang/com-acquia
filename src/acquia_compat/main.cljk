(ns acquia-compat.main
  "Kotodama WASM entrypoint for the Acquia clean-room actor (L4) — Clojure port.
  ...
  facts (`acquia.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [kotoba.lang.text :as str]))

(def ns-prefix "acquia")
(def tier "L4")
...
;; --- kotoba paginate kernel twins (src/acquia_compat/paginate.kotoba) ---
(defn as-int-kernel
  ...
  of acquia-compat.main/as-int stays here as the retained oracle."
  [v]
  (if (pos? v) v 0))
...
(defn emit-facts
  "EAVT facts for one record: {\"acquia.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "acquia-compat" :tier tier :entities entities} 200])
...
(defn start! [] :acquia-compat/ready)
